...
const Display = ({ input, output }) => (
  <div className="output">
    <span className="result">
      {output.length >= 13
        ? `${parseInt(output).toExponential()}`
        : output.charAt(0) === "0" && output.length > 1
        ? output.substring(1)
        : output}
    </span>
    <span id="display" className="input">
      {input.length >= 13
        ? `${parseInt(input).toExponential()}`
        : input.charAt(0) === "0" && input.length > 1
        ? input.substring(1)
        : input}
    </span>
  </div>
...
const Key = ({ keyData: { id, value }, handleInput }) => (
  <motion.button whileTap={{ scale: 1.05 }} id={id} onClick={() => handleInput(value)}>
    {value}
  </motion.button>
);
    ...
    if (!calculatorData.length) {
      ...
      setCalculatorData("0.");
        ...
        if (lastChat === "*" || operators.includes(lastChat)) {
            ...
            const formattedValue = lastChat === "." || input.includes(".") ? `${calculatorData}` : `${calculatorData}.`;
